...
module kt.palette {
    ...
    export function buildPalette() {
        ...
        for (let n in kt.graph.po_node.PoStates) {
            let c = kt.graph.po_node.PoStates[n];
            if (typeof c === 'string') {


                for (let m in kt.graph.po_node.PoDischargeTypes) {
                    let d = kt.graph.po_node.PoDischargeTypes[m];
                    if (typeof d === 'string')
